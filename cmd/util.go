...
var CommonFlags []cli.Flag = []cli.Flag{
	...
	&cli.StringFlag{
		Name:    "ownership",
		Usage:   "Ledger ownership enforcement mode: 'enforce' (sign + reject unauthorized writes, default), 'observe' (sign + log violations), or 'off' (legacy opt-out). Aliases: 'log' and 'log-only' map to observe; 'on' maps to enforce. Invalid values fail fast. All nodes on network must use the same mode (breaking wire format change).",
